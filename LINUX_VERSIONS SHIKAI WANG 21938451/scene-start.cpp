...
int N_object = 1; // store how many object
//----------------------------------------------------------------------------
//
...
// Part J.1 Create a delete function for current object
static void deleteObject(int id){
    if(sceneObjs[currObject].meshId != 0 && sceneObjs[currObject].meshId!=55){
        sceneObjs[currObject].meshId = NULL; //set current object to null
        nObjects--;
        if(currObject > 3){
...
// Part J.2 Create a duplicate object
static void dulicateObject(int id){
    //No over 1024 object
...
void init( void )
{
    ...
    //Part I : initial second sphere 
    addObject(55); //this is second Sphere for the second light
...
void drawMesh(SceneObject sceneObj)
{
    ...
    // Part B ObjectRotateXYZ 
    // Set the model matrix - this should combine translation, rotation and scaling based on what's
    // in the sceneObj structure (see near the top of the program).

    mat4 rotate = RotateX(-sceneObj.angles[0]) * RotateY(sceneObj.angles[1]) * RotateZ(-sceneObj.angles[2]);
    mat4 model_mx = Translate(sceneObj.loc) * rotate * Scale(sceneObj.scale);


    // Set the model-view matrix for the shaders
    glUniformMatrix4fv( modelViewU, 1, GL_TRUE, view * model_mx );
...
void display( void )
{
    ...
    view *= xrotate * yrotate;

    //Part I : light 1 and light 2
...
//Part C : Material Menu item for adjusting the amounts of ambient, diffuse, specular light, and shine
static void ambient_diffuse(vec2 amb_diffuse){
    sceneObjs[toolObj].ambient += amb_diffuse[0];
    sceneObjs[toolObj].diffuse += amb_diffuse[1];
}

static void specularlight_shine(vec2 lit_shine){
    sceneObjs[toolObj].specular += lit_shine[0];
    sceneObjs[toolObj].shine += lit_shine[1];
}
static void materialMenu(int id)
{
    ...
    if (id==10) {
        //indicate to current object after choose
        toolObj = currObject;
        ...
        //indicate to current object after choose
        toolObj = currObject;
        setToolCallbacks(ambient_diffuse, mat2(1,0,0,1),
        specularlight_shine,mat2(1,0,0,1));
    }
...
static void mainmenu(int id)
{
    ...
    //Part J.1: Delete current Object
    if(id == 90 && currObject >=0 ){
        deleteObject(currObject);}
    //Part J.2: Duplicate cuurent Object
    if( id == 91 && currObject >=0){
        dulicateObject(currObject);}
...
static void makeMenu()
{
    ...
    //Part J: add Delete current obj, Duplicate, spot light
    glutAddMenuEntry("Delete Current Object",90);
    glutAddMenuEntry("Duplicate current object",91);
...
void reshape( int width, int height )
{
    ...
    GLfloat nearDist = 0.01; //closer view

    //Part E : reshape the object each time the window is resized
    if(width>height){
